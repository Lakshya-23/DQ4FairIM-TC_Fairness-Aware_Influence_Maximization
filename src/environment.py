...
class FairIMEnvironment:
    ...
    def __init__(
        ...
        lambda_disparity: float = 0.0,   # disparity penalty weight: penalises (max_comm - min_comm)
        seed: int = 42,
    ):
    ...
    def _compute_reward(self, new_node: int):
        ...
        # ── Maximin fairness term (DQ4FairIM Eq. 6) ──────────────────────
        # Rewards improving the worst-off community's score
        delta_fairness = after["fairness"] - before["fairness"]

        # ── Disparity penalty ─────────────────────────────────────────────
        # Penalises widening the gap between best and worst community.
        # disparity = max(per_comm) - min(per_comm)
        # delta_disparity > 0 means the gap GREW  → reward decreases
        # delta_disparity < 0 means the gap SHRANK → reward increases
        disparity_before = 0.0
        disparity_after  = 0.0
        if before.get("per_comm"):
            vals_b = list(before["per_comm"].values())
            if len(vals_b) > 1:
                disparity_before = max(vals_b) - min(vals_b)
        if after.get("per_comm"):
            vals_a = list(after["per_comm"].values())
            if len(vals_a) > 1:
                disparity_after = max(vals_a) - min(vals_a)
        delta_disparity = disparity_after - disparity_before

        # ── Latency penalty (only active when deadline is set) ────────────
        # latency_penalty = 0.0
        # if self.deadline and after["per_comm"]:
        #     vals = list(after["per_comm"].values())
        #     if vals:
        #         latency_penalty = max(vals) - min(vals)

        latency_penalty = 0.0
        if self.deadline and "avg_latency" in after:
            # Penalise based on how close the average activation time is to the deadline
            latency_penalty = after["avg_latency"]

        # ── Combined reward ───────────────────────────────────────────────
        # R = delta_reach
        #   + phi          * delta_fairness    (lift worst-off community)
        #   - lambda_d     * delta_disparity   (close the inter-community gap)
        #   - gamma_latency* latency_penalty   (time-critical mode only)
        reward = (delta_reach
                  + self.phi             * delta_fairness
                  - self.lambda_disparity * delta_disparity
        ...
        info = {**after,
                ...
                "delta_disparity":  delta_disparity}
        return reward, info
